...
class LbSkinningGpu:
    ...
    def __init__(self, characterFilePath = '', dofs = None, skinningWeights = None, miniBatchSize = 0, nodeName=''):
        ...
        if(characterFilePath != '' and dofs is not None and miniBatchSize != 0  and nodeName != '' and skinningWeights is not None):
            ...
            raise ValueError('Invalid argument during the construction of the lb skinning operator!')

    # return the node for building a tf graph
    def getNode(self):

        return [self.lbSkinningGpuOperator[0],self.lbSkinningGpuOperator[1]]

########################################################################################################################
# Register gradients
########################################################################################################################

@ops.RegisterGradient("LbSkinningGpu")
def lb_skinning_gpu_grad(op, grad1, grad2, grad3, grad4, grad5, grad6):

    dofsGrad, skinningWeightsGrad = customOperators.lb_skinning_gpu_grad(grad1,
                                                    op.outputs[0],
                                                    op.outputs[2],
                                                    op.outputs[3],
                                                    op.outputs[4],
