...
__global__ void initializeDevice(CUDABasedRasterizationInput input)
{
	...
	if (idx<input.w*input.h*input.numberOfCameras)
	{
		...
		input.d_depthSecondBuffer[idx] = INT_MAX;
		input.d_depthThirdBuffer[idx] = INT_MAX;
		input.d_depthFourthBuffer[idx] = INT_MAX;
		input.d_depthFifthBuffer[idx] = INT_MAX;
		input.d_depthSixthBuffer[idx] = INT_MAX;
		input.d_depthSeventhBuffer[idx] = INT_MAX;
		input.d_depthEighthBuffer[idx] = INT_MAX;
		input.d_depthNinthBuffer[idx] = INT_MAX;
		input.d_depthTenthBuffer[idx] = INT_MAX;
		...
		input.d_renderSecondBuffer[3 * idx + 0] = 0.f;
		input.d_renderSecondBuffer[3 * idx + 1] = 0.f;
		input.d_renderSecondBuffer[3 * idx + 2] = 0.f;

		input.d_renderThirdBuffer[3 * idx + 0] = 0.f;
		input.d_renderThirdBuffer[3 * idx + 1] = 0.f;
		input.d_renderThirdBuffer[3 * idx + 2] = 0.f;

		input.d_renderFourthBuffer[3 * idx + 0] = 0.f;
		input.d_renderFourthBuffer[3 * idx + 1] = 0.f;
		input.d_renderFourthBuffer[3 * idx + 2] = 0.f;

		input.d_renderFifthBuffer[3 * idx + 0] = 0.f;
		input.d_renderFifthBuffer[3 * idx + 1] = 0.f;
		input.d_renderFifthBuffer[3 * idx + 2] = 0.f;

		input.d_renderSixthBuffer[3 * idx + 0] = 0.f;
		input.d_renderSixthBuffer[3 * idx + 1] = 0.f;
		input.d_renderSixthBuffer[3 * idx + 2] = 0.f;

		input.d_renderSeventhBuffer[3 * idx + 0] = 0.f;
		input.d_renderSeventhBuffer[3 * idx + 1] = 0.f;
		input.d_renderSeventhBuffer[3 * idx + 2] = 0.f;

		input.d_renderEighthBuffer[3 * idx + 0] = 0.f;
		input.d_renderEighthBuffer[3 * idx + 1] = 0.f;
		input.d_renderEighthBuffer[3 * idx + 2] = 0.f;

		input.d_renderNinthBuffer[3 * idx + 0] = 0.f;
		input.d_renderNinthBuffer[3 * idx + 1] = 0.f;
		input.d_renderNinthBuffer[3 * idx + 2] = 0.f;

		input.d_renderTenthBuffer[3 * idx + 0] = 0.f;
		input.d_renderTenthBuffer[3 * idx + 1] = 0.f;
		input.d_renderTenthBuffer[3 * idx + 2] = 0.f;
...
__global__ void renderDepthBufferDevice(CUDABasedRasterizationInput input, int mode)
{
	const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
		...
		for (int u = input.d_BBoxes[idx].x; u <= input.d_BBoxes[idx].z; u++)
		{
			for (int v = input.d_BBoxes[idx].y; v <= input.d_BBoxes[idx].w; v++)
			{
				...
				if (isInsideTriangle)
				{
					 ...
					 */
/*if (input.enableDepthPeel) {
                        int old_smallest = atomicMin(&input.d_depthBuffer[pixelId], z);
                        if (z < old_smallest) {
                            atomicExch(&input.d_depthSecondBuffer[pixelId],old_smallest);
                        }
                        else if (old_smallest < z && z < input.d_depthSecondBuffer[pixelId]) {
                            atomicExch(&input.d_depthSecondBuffer[pixelId],z);
                        }
					}
					else {
					...
					else if ((mode==2)&&(z > input.d_depthBuffer[pixelId])) {
                        atomicMin(&input.d_depthSecondBuffer[pixelId], z);
					}
					else if ((mode==3)&&(z > input.d_depthSecondBuffer[pixelId])) {
                        atomicMin(&input.d_depthThirdBuffer[pixelId], z);
					}
					else if ((mode==4)&&(z > input.d_depthThirdBuffer[pixelId])) {
                        atomicMin(&input.d_depthFourthBuffer[pixelId], z);
					}
					else if ((mode==5)&&(z > input.d_depthFourthBuffer[pixelId])) {
                        atomicMin(&input.d_depthFifthBuffer[pixelId], z);
					}
					else if ((mode==6)&&(z > input.d_depthFifthBuffer[pixelId])) {
                        atomicMin(&input.d_depthSixthBuffer[pixelId], z);
					}
					else if ((mode==7)&&(z > input.d_depthSixthBuffer[pixelId])) {
                        atomicMin(&input.d_depthSeventhBuffer[pixelId], z);
					}
					else if ((mode==8)&&(z > input.d_depthSeventhBuffer[pixelId])) {
                        atomicMin(&input.d_depthEighthBuffer[pixelId], z);
					}
					else if ((mode==9)&&(z > input.d_depthEighthBuffer[pixelId])) {
                        atomicMin(&input.d_depthNinthBuffer[pixelId], z);
					}
					else if ((mode==10)&&(z > input.d_depthNinthBuffer[pixelId])) {
                        atomicMin(&input.d_depthTenthBuffer[pixelId], z);
					}
				}
			}
		}
	...
	if (idx < input.numberOfCameras * input.F)
	{
		...
		for (int u = input.d_BBoxes[idx].x; u <= input.d_BBoxes[idx].z; u++)
		{
			for (int v = input.d_BBoxes[idx].y; v <= input.d_BBoxes[idx].w; v++)
			{
                ...
                if (isInsideTriangle)
                {
                    if ((int)z == input.d_depthBuffer[pixelId])
                    {
                    ...
                    //
                    //if (isInsideTriangle && (int)z == input.d_depthSecondBuffer[pixelId]) {
                    else
                    {
                        bool mode2 = ((int)z == input.d_depthSecondBuffer[pixelId]);
                        bool mode3 = ((int)z == input.d_depthThirdBuffer[pixelId]);
                        bool mode4 = ((int)z == input.d_depthFourthBuffer[pixelId]);
                        bool mode5 = ((int)z == input.d_depthFifthBuffer[pixelId]);
                        bool mode6 = ((int)z == input.d_depthSixthBuffer[pixelId]);
                        bool mode7 = ((int)z == input.d_depthSeventhBuffer[pixelId]);
                        bool mode8 = ((int)z == input.d_depthEighthBuffer[pixelId]);
                        bool mode9 = ((int)z == input.d_depthNinthBuffer[pixelId]);
                        bool mode10 = ((int)z == input.d_depthTenthBuffer[pixelId]);


                        if (mode2 || mode3 || mode4 || mode5 || mode6 || mode7 || mode8 || mode9 || mode10)
                        {
                            int pixelId1 = 2 * idc* input.w * input.h + 2 * input.w * v + 2 * u;
                            int pixelId2 = 3 * idc* input.w * input.h + 3 * input.w * v + 3 * u;
                            float3 color = make_float3(0.f,0.f,0.f);
                            if (input.albedoMode == AlbedoMode::UV)
                            {
                                float2 texCoord0 = make_float2(input.d_textureCoordinates[idf * 3 * 2 + 0 * 2 + 0], 1.f - input.d_textureCoordinates[idf * 3 * 2 + 0 * 2 + 1]);
                                float2 texCoord1 = make_float2(input.d_textureCoordinates[idf * 3 * 2 + 1 * 2 + 0], 1.f - input.d_textureCoordinates[idf * 3 * 2 + 1 * 2 + 1]);
                                float2 texCoord2 = make_float2(input.d_textureCoordinates[idf * 3 * 2 + 2 * 2 + 0], 1.f - input.d_textureCoordinates[idf * 3 * 2 + 2 * 2 + 1]);
                            ...
                            if (mode2)
                            {
                                input.d_renderSecondBuffer[pixelId2 + 0] = color.x;
                                input.d_renderSecondBuffer[pixelId2 + 1] = color.y;
                                input.d_renderSecondBuffer[pixelId2 + 2] = color.z;
                            }
                            else if (mode3)
                            {
                                input.d_renderThirdBuffer[pixelId2 + 0] = color.x;
                                input.d_renderThirdBuffer[pixelId2 + 1] = color.y;
                                input.d_renderThirdBuffer[pixelId2 + 2] = color.z;
                            }
                            else if (mode4)
                            {
                                input.d_renderFourthBuffer[pixelId2 + 0] = color.x;
                                input.d_renderFourthBuffer[pixelId2 + 1] = color.y;
                                input.d_renderFourthBuffer[pixelId2 + 2] = color.z;
                            }
                            else if (mode5)
                            {
                                input.d_renderFifthBuffer[pixelId2 + 0] = color.x;
                                input.d_renderFifthBuffer[pixelId2 + 1] = color.y;
                                input.d_renderFifthBuffer[pixelId2 + 2] = color.z;
                            }
                            else if (mode6)
                            {
                                input.d_renderSixthBuffer[pixelId2 + 0] = color.x;
                                input.d_renderSixthBuffer[pixelId2 + 1] = color.y;
                                input.d_renderSixthBuffer[pixelId2 + 2] = color.z;
                            }
                            else if (mode7)
                            {
                                input.d_renderSeventhBuffer[pixelId2 + 0] = color.x;
                                input.d_renderSeventhBuffer[pixelId2 + 1] = color.y;
                                input.d_renderSeventhBuffer[pixelId2 + 2] = color.z;
                            }
                            else if (mode8)
                            {
                                input.d_renderEighthBuffer[pixelId2 + 0] = color.x;
                                input.d_renderEighthBuffer[pixelId2 + 1] = color.y;
                                input.d_renderEighthBuffer[pixelId2 + 2] = color.z;
                            }
                            else if (mode9)
                            {
                                input.d_renderNinthBuffer[pixelId2 + 0] = color.x;
                                input.d_renderNinthBuffer[pixelId2 + 1] = color.y;
                                input.d_renderNinthBuffer[pixelId2 + 2] = color.z;
                            }
                            else if (mode10)
                            {
                                input.d_renderTenthBuffer[pixelId2 + 0] = color.x;
                                input.d_renderTenthBuffer[pixelId2 + 1] = color.y;
                                input.d_renderTenthBuffer[pixelId2 + 2] = color.z;
                            }
                        }
                    }
                //
...
/*for (int mode = 1; mode <= 10; mode++) {
		    renderDepthBufferDevice << <(input.F*input.numberOfCameras + THREADS_PER_BLOCK_CUDABASEDRASTERIZER - 1) / THREADS_PER_BLOCK_CUDABASEDRASTERIZER, THREADS_PER_BLOCK_CUDABASEDRASTERIZER >> >(input, mode);
		}*//*
