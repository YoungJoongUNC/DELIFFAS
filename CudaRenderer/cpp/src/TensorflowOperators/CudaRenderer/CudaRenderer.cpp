...
.Output("render_second_buffer: float")
...
.Attr("enable_depth_peel: bool = false");
	...
	OpKernel(context) 
{
	...
	bool enableDepthPeel;
	OP_REQUIRES_OK(context, context->GetAttr("enable_depth_peel", &enableDepthPeel));
	...
	std::cout << "Enable Depth Peel : " << enableDepthPeel << std::endl;
	...
	cudaBasedRasterization = new CUDABasedRasterization(faces, textureCoordinates, numberOfPoints, numberOfCameras, renderResolutionU, renderResolutionV, albedoMode, shadingMode, computeNormal, enableDepthPeel);
}
...
void CudaRenderer::setupInputOutputTensorPointers(OpKernelContext* context)
{
    ...
    //[5]
	//render second depth buffer
	tensorflow::Tensor* outputTensorRenderSecond;
	OP_REQUIRES_OK(context, context->allocate_output(5, tensorflow::TensorShape(channel3DimSize), &outputTensorRenderSecond));
	Eigen::TensorMap<Eigen::Tensor<float, 1, 1, Eigen::DenseIndex>, 16> outputTensorRenderSecondFlat = outputTensorRenderSecond->flat<float>();
	d_outputRenderSecondBuffer = outputTensorRenderSecondFlat.data();
...
void CudaRenderer::Compute(OpKernelContext* context)
{
    ...
    for (int b = 0; b < numberOfBatches; b++)
    {
        ...
        cudaBasedRasterization->set_D_renderSecondBuffer(			d_outputRenderSecondBuffer				+ b * numberOfCameras * renderResolutionV * renderResolutionU * 3);
